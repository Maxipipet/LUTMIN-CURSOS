// =============================================================
// LUTMIN V32.0 · WORKSPACE-AWARE MODULE LOADER
...
(function(){
  ...
  const scriptFiles=[
    ...
    'assets/js/modules/core/admin-demand-v32.js'
  ];
  ...
  const F={
    ...
    adminDemand:'assets/js/modules/core/admin-demand-v32.js'
  };
  ...
  function loadCssOnce(file){
    ...
    return new Promise(resolve=>{const link=document.createElement('link');link.rel='stylesheet';link.href=withVersion(file);link.dataset.lutminV30='css';let settled=false;const done=()=>{if(settled)return;settled=true;state.loaded.add(key);resolve(true);};link.onload=done;link.onerror=()=>{console.warn('[Lutmin V32] CSS no disponible:',file);done();};document.head.appendChild(link);setTimeout(done,4500);});
  }
  ...
  async function registerServiceWorker(){
    ...
    try{const reg=await navigator.serviceWorker.register(`./sw.js?v=${VERSION}`,{scope:'./'});window.dispatchEvent(new CustomEvent('lutmin:v30:sw-registered',{detail:{registration:reg}}));reg.update().catch(()=>{});return reg;}catch(err){console.warn('[Lutmin V32] Service Worker no disponible:',err?.message||err);return false;}
  }
