...
(function(){
  ...
  async function fetchView(name){
    ...
    const promise=(async()=>{
      try{
        ...
        const bind=()=>window.LutminV31CoreBindings?.bind?.(name);
        if(!bind())setTimeout(bind,0);
        window.dispatchEvent(new CustomEvent('lutmin:v31:view-ready',{detail:{view:name}}));return true;
      }catch(err){
        console.error('[Lutmin V31] Vista no disponible:',name,err);target.dataset.lutminViewState='error';
        target.innerHTML=`<div class="rounded-2xl border border-amber-200 bg-amber-50 p-5 text-sm text-amber-900"><strong>No pude cargar esta sección.</strong><br><span class="text-xs">Revisá la conexión y volvé a intentar.</span><div class="mt-3"><button type="button" onclick="window.LutminV31Views.retry('${name}')" class="px-4 py-2 rounded-xl bg-amber-900 text-white text-xs font-bold">Reintentar</button></div></div>`;
  ...
  window.LutminV31Views={version:VERSION,ensure:fetchView,ensureForRole,ensureForTab,retry,prefetch,isLoaded:loaded,status};
})();
