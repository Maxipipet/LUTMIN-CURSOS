    ...
    // Tailwind se compila estáticamente en V2.3 para mejorar rendimiento.

    // =========================================================
    // V2.0 · ANALÍTICA PÚBLICA SIN COOKIES / SIN SERVICIOS PAGOS
    // =========================================================
    ...
    window.addEventListener('load',()=>lutminIdleV23(()=>trackPublicEventV20('page_view',{title:document.title}),2000));
    ...
    lutminIdleV23(()=>ensurePublicMetricsPanelV20(),1800);
    ...
    // =========================================================
    // V2.3 · RENDIMIENTO, CACHÉ, CARGA DIFERIDA Y DIAGNÓSTICO
    // =========================================================
    const LUTMIN_V23_VERSION='2.3.0';
    const lutminPerfV23={requests:0,totalMs:0,slow:[],startedAt:performance.now()};
    function lutminCacheGet(key,ttl){try{const raw=sessionStorage.getItem('lutmin-cache:'+key);if(!raw)return null;const x=JSON.parse(raw);if(Date.now()-x.ts>ttl){sessionStorage.removeItem('lutmin-cache:'+key);return null;}return x.data;}catch(_){return null;}}
    function lutminCacheSet(key,data){try{sessionStorage.setItem('lutmin-cache:'+key,JSON.stringify({ts:Date.now(),data}));}catch(_){}}
    function loadScriptV23(src,test){if(test?.())return Promise.resolve();window.__lutminScriptLoads=window.__lutminScriptLoads||{};if(window.__lutminScriptLoads[src])return window.__lutminScriptLoads[src];window.__lutminScriptLoads[src]=new Promise((resolve,reject)=>{const el=document.createElement('script');el.src=src;el.async=true;el.onload=resolve;el.onerror=reject;document.head.appendChild(el);});return window.__lutminScriptLoads[src];}
    async function ensureJsPdfV23(){try{await loadScriptV23('https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js',()=>!!window.jspdf?.jsPDF);}catch(e){console.error('jsPDF',e);}}
    async function ensureQrCodeV23(){try{await loadScriptV23('https://cdnjs.cloudflare.com/ajax/libs/qrcodejs/1.0.0/qrcode.min.js',()=>!!window.QRCode);}catch(e){console.error('QRCode',e);}}
    function lutminIdleV23(fn,timeout=1600){if('requestIdleCallback'in window)return requestIdleCallback(fn,{timeout});return setTimeout(fn,Math.min(timeout,500));}
    function debounceV23(fn,wait=180){let t;return(...args)=>{clearTimeout(t);t=setTimeout(()=>fn(...args),wait);};}
    window.lutminRuntimeV23=()=>({version:LUTMIN_V23_VERSION,requests:lutminPerfV23.requests,avgRequestMs:lutminPerfV23.requests?Math.round(lutminPerfV23.totalMs/lutminPerfV23.requests):0,slowRequests:lutminPerfV23.slow.slice(-10),domReadyMs:Math.round(performance.now())});
    const lutminReportedErrorsV23=new Map();
    async function reportClientErrorV23(payload){
      try{
        window.__lutminLastError=payload;
        if(!supabaseClient||!currentLutminUser?.id)return;
        const key=(payload.message||'')+'|'+(payload.source||'')+'|'+(payload.line||0);
        const now=Date.now(); if(now-(lutminReportedErrorsV23.get(key)||0)<60000)return; lutminReportedErrorsV23.set(key,now);
        await supabaseClient.from('client_error_logs').insert({user_id:currentLutminUser.id,message:String(payload.message||'Error').slice(0,1000),source:String(payload.source||'').slice(0,500),line_number:Number(payload.line||0),stack:String(payload.stack||'').slice(0,5000),app_version:LUTMIN_V23_VERSION,user_agent:navigator.userAgent.slice(0,800)});
      }catch(_){ }
    }
    window.addEventListener('error',e=>reportClientErrorV23({message:e.message||'Error',source:e.filename||'',line:e.lineno||0,stack:e.error?.stack||'',at:new Date().toISOString()}));
    window.addEventListener('unhandledrejection',e=>reportClientErrorV23({message:String(e.reason?.message||e.reason||'Promise rechazada'),source:'promise',line:0,stack:e.reason?.stack||'',at:new Date().toISOString()}));
    ...
    const lutminTimedFetchV23=async (...args)=>{const t0=performance.now();try{return await fetch(...args);}finally{const ms=performance.now()-t0;lutminPerfV23.requests++;lutminPerfV23.totalMs+=ms;if(ms>1200)lutminPerfV23.slow.push({ms:Math.round(ms),url:String(args[0]||'').slice(0,140),at:new Date().toISOString()});}};
    const supabaseClient = supabaseConfigured
      ? window.supabase.createClient(SUPABASE_URL, SUPABASE_PUBLISHABLE_KEY,{global:{fetch:lutminTimedFetchV23},auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true}})
      : null;

    let currentLutminUser = null;
    let currentAccessMode = sessionStorage.getItem('lutmin-access-mode') || null;
    let currentAccessContext = { student:false, admin:false, companies:[] };
    let supportTickets = [];
    let supportMessages = [];
    ...
    document.addEventListener('keydown', event => {
      if (event.key === 'Escape') {
        ['courseModal','interestModal','campusModal','lessonModal','assessmentModal','publicTalentModal','profileModal','accessSwitcherModal','adminAccountPasswordModal','supportThreadModal','certificateModal','authModal','passwordModal','adminCourseEditModal','adminLessonEditModal','adminStudentDetailModal','leadConvertModal','paymentModal','adminCompanyEditModal','attendanceModal'].forEach(id => {
    ...
    async function loadPublicSiteStatsV21() {
      if (!supabaseClient) return;
      try {
        const cached = lutminCacheGet('public-site-stats-v23', 300000);
        const result = cached ? { data: cached, error: null } : await supabaseClient.rpc('get_public_site_stats');
        const { data, error } = result;
        if (error || !data) return;
        if (!cached) lutminCacheSet('public-site-stats-v23', data);
    ...
    async function loadPublicCatalog() {
      if (!supabaseClient) return;
      const cached = lutminCacheGet('public-catalog-v23', 300000);
      const result = cached ? { data: cached, error: null } : await supabaseClient.rpc('get_public_course_catalog');
      const { data, error } = result;
      if (error) {
        console.error('Catálogo público:', error);
        return; // dejamos las tarjetas de respaldo del HTML si todavía no se ejecutó Etapa 10
      }
      publicCatalog = Array.isArray(data) ? data : [];
      if (!cached) lutminCacheSet('public-catalog-v23', publicCatalog);
      renderPublicCatalog();
    }
    ...
    function renderPublicCatalog() {
      ...
      grid.innerHTML = publicCatalog.map(course => {
        ...
        return `<article data-category="${escapeHtml(course.category || 'tecnico')}" class="course-card bg-white rounded-3xl overflow-hidden border border-slate-100 shadow-sm hover:shadow-soft transition group">
          <div class="h-52 overflow-hidden relative"><img loading="lazy" decoding="async" src="${publicCourseImage(course.category)}" alt="${escapeHtml(course.title)}" class="w-full h-full object-cover group-hover:scale-105 transition duration-500" loading="lazy" decoding="async"><span class="absolute top-4 left-4 bg-white/95 px-3 py-1 rounded-full text-xs font-extrabold text-lutmin-dark">${categoryLabel(course.category)}</span>${course.featured?'<span class="absolute top-4 right-4 bg-lutmin-dark/95 text-white px-3 py-1 rounded-full text-[10px] font-extrabold uppercase tracking-wide"><i class="fa-solid fa-star text-amber-300 mr-1"></i>Destacado</span>':''}</div>
    ...
    function openPublicCourse(courseId) {
      ...
      offersArea.innerHTML = offers.length
        ? `<h4 class="font-extrabold text-lutmin-dark">Próximas ediciones</h4><div class="mt-3 space-y-3">${offers.map(o => `<div class="rounded-2xl border border-blue-100 bg-blue-50 p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3"><div><p class="font-bold text-lutmin-dark">${formatPublicDate(o.start_date)} · ${escapeHtml(String(o.modality || '').replace(/^./, x => x.toUpperCase()))}</p><p class="mt-1 text-xs text-slate-600">${o.location ? `${escapeHtml(o.location)} · ` : ''}${formatPublicMoney(o.price, o.currency)}${o.available_seats === null ? '' : ` · ${o.available_seats} cupos`}</p></div>${Number(o.available_seats)===0?`<button onclick="openWaitlistV22('${course.id}','${o.id}')" class="px-4 py-2.5 rounded-xl bg-amber-500 text-white text-xs font-extrabold"><i class="fa-solid fa-hourglass-half mr-1"></i>Lista de espera</button>`:`<button onclick="openInterestModal('${course.id}','${o.id}')" class="px-4 py-2.5 rounded-xl bg-lutmin-light text-white text-xs font-extrabold">Me interesa esta edición</button>`}</div>`).join('')}</div>`
    ...
    async function loadAccessContext() {
      if (!supabaseClient) return {student:false,admin:false,companies:[]};
      const { data, error } = await supabaseClient.rpc('get_my_access_context');
      if (error) { console.error('Access context',error); return {student:false,admin:false,companies:[]}; }
      return { student:Boolean(data?.student), admin:Boolean(data?.admin), companies:Array.isArray(data?.companies)?data.companies:[] };
    }

    function accessCount(ctx=currentAccessContext) {
      return (ctx.student?1:0)+(ctx.admin?1:0)+(ctx.companies?.length?1:0);
    }
    ...
    document.getElementById('authForm').addEventListener('submit', async event => {
      ...
      try {
        ...
        showToast(currentLutminUser.role==='company_admin'?'Acceso Empresa abierto.':currentLutminUser.role==='admin'?'Administración abierta.':'Acceso Alumno abierto.');
      } catch(error) {
    ...
    async function loadCurrentLutminUser(user, requestedMode = null) {
      ...
      if(mode==='student'&&!currentAccessContext.student&&!currentAccessContext.admin){showToast('Este correo no tiene acceso Alumno habilitado.');return false;}
      if(!mode){ mode=currentAccessContext.admin?'admin':currentAccessContext.student?'student':currentAccessContext.companies.length?'company':null; }
      if(mode==='student'&&currentAccessContext.admin) mode='admin';
      if(mode==='admin'&&!currentAccessContext.admin) mode=currentAccessContext.student?'student':currentAccessContext.companies.length?'company':null;
      ...
      const effectiveRole=mode==='company'?'company_admin':mode==='admin'?'admin':'student';
      ...
      if(effectiveRole==='company_admin'){await loadCompanyPortalData();goToCampusTab('company');}
      else if(effectiveRole==='admin'){goToCampusTab('admin'); await loadAdminData();}
      else {await loadCampusData();await loadStudentAgenda();}
      await loadNotificationCenter();
      if(currentLutminUser.mustChangePassword&&effectiveRole!=='admin') setTimeout(()=>openPasswordModal('first'),50);
      return true;
    }
    ...
    function paintCurrentLutminUser() {
      ...
      const isAdmin=currentLutminUser.role==='admin', isCompanyAdmin=currentLutminUser.role==='company_admin';
      ...
      document.getElementById('companyDesktopTab')?.classList.toggle('hidden',!isCompanyAdmin); document.getElementById('companyMobileTab')?.classList.toggle('hidden',!isCompanyAdmin);
      document.querySelectorAll('[data-student-only="true"]').forEach(el=>el.classList.toggle('hidden',isCompanyAdmin||isAdmin));
      document.querySelectorAll('[data-talent-tab="true"]').forEach(el=>el.classList.toggle('hidden',currentLutminUser.role!=='student'));
      const roleLabel=document.getElementById('campusSidebarRole'); if(roleLabel)roleLabel.textContent=isAdmin?'Administrador':isCompanyAdmin?'Empresa':'Alumno';
      ...
      if(profileSubtitle)profileSubtitle.textContent=isAdmin?'Cuenta administradora de Lutmin':isCompanyAdmin?'Responsable de empresa':'Perfil profesional de alumno';
      if(profileStatus)profileStatus.textContent=isCompanyAdmin?'Acceso corporativo':isAdmin?'Administración':'Disponible';
      if(profileCertCount)profileCertCount.classList.toggle('hidden',isCompanyAdmin||isAdmin);
    ...
    async function switchAccessMode(mode){
      ...
      goToCampusTab(mode==='company'?'company':mode==='admin'?'admin':'dashboard');
    }

    async function logoutLutmin() {
      if (supabaseClient) await supabaseClient.auth.signOut();
      currentLutminUser = null;
      currentAccessMode = null; currentAccessContext = {student:false,admin:false,companies:[]}; sessionStorage.removeItem('lutmin-access-mode');
    ...
    if (supabaseClient) {
      supabaseClient.auth.onAuthStateChange((event, session) => {
        if (session?.user) loadCurrentLutminUser(session.user, currentAccessMode);
    ...
    document.querySelectorAll('.campus-tab').forEach(button => {
      button.addEventListener('click', async () => {
        ...
        if (tab === 'company' && currentLutminUser?.role !== 'company_admin') return;
        if (currentLutminUser?.role === 'company_admin' && !['company','profile','notifications','support'].includes(tab)) return;
        goToCampusTab(tab);
    ...
    document.getElementById('passwordForm')?.addEventListener('submit', async event => {
      ...
      if (error) {
        console.error(error); status.textContent = error.message || 'No se pudo cambiar la contraseña.';
        btn.disabled = false; btn.textContent = 'Guardar contraseña'; return;
      }
      try { await supabaseClient.rpc('finish_first_password_change'); } catch (_) {}
    ...
    async function loadCampusData() {
      ...
      if (enrollError) {
        ...
        showToast('No pudimos cargar tus cursos. Si recién configuraste la base, ejecutá el SQL de Etapa 2.');
        return;
      }
      ...
      if (courseIds.length) {
        const { data, error } = await supabaseClient
          .from('lessons')
          .select('id,course_id,title,description,content,duration_minutes,sort_order,video_url,material_url,material_path')
      ...
      if (courseIds.length) {
        const { data: assessmentRows, error: assessmentError } = await supabaseClient
          .from('assessments')
          .select('id,course_id,title,description,passing_score,published')
          .in('course_id', courseIds)
          .eq('published', true);
      ...
      const { data: certificateRows, error: certificateError } = await supabaseClient
        .from('certificates')
        .select('id,user_id,course_id,code,full_name,course_title,duration_hours,score,status,issued_at,revoked_at,revoked_reason')
        .eq('user_id', currentLutminUser.id)
      ...
      campusCourseSummaries = validEnrollments.map(enrollment => {
        const courseLessons = lessons
          .filter(lesson => lesson.course_id === enrollment.course.id)
          .sort((a, b) => a.sort_order - b.sort_order);
        const completedLessons = courseLessons.filter(lesson => completedSet.has(lesson.id));
        const total = courseLessons.length;
        const completed = completedLessons.length;
        const percent = total ? Math.round((completed / total) * 100) : 0;
        const nextLesson = courseLessons.find(lesson => !completedSet.has(lesson.id)) || null;
        const remainingMinutes = courseLessons
    ...
    function renderCertificatesGrid() {
      ...
      grid.innerHTML = campusCertificates.map(cert => {
        ...
        return `<button onclick="openOwnedCertificate('${cert.id}')" class="text-left bg-white p-5 sm:p-6 rounded-3xl border ${valid ? 'border-slate-100 hover:border-lutmin-light' : 'border-red-100'} transition">
          <div class="flex items-start justify-between gap-4"><div><p class="text-[10px] uppercase tracking-widest font-bold ${valid ? 'text-green-600' : 'text-red-600'}">${valid ? 'Válido' : 'Revocado'}</p><h3 class="mt-1 font-extrabold text-lutmin-dark">${escapeHtml(cert.course_title)}</h3><p class="mt-2 text-xs text-slate-500">${Math.round(Number(cert.score))}% · ${formatCertificateDate(cert.issued_at)}</p></div><i class="fa-solid fa-award text-2xl ${valid ? 'text-lutmin-light' : 'text-red-400'}"></i></div>
    ...
    async function showCertificate(cert, options = {}) {
      ...
      document.getElementById('certificateStatusLabel').textContent = valid ? 'Certificado válido' : 'Certificado revocado';
      document.getElementById('certificateStatusLabel').className = `mt-7 text-[10px] sm:text-xs uppercase tracking-[.2em] font-extrabold ${valid ? 'text-green-600' : 'text-red-600'}`;
      document.getElementById('certificateStatusIcon').className = `w-12 h-12 sm:w-14 sm:h-14 rounded-full flex items-center justify-center text-xl shrink-0 ${valid ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'}`;
      document.getElementById('certificateStatusIcon').innerHTML = `<i class="fa-solid ${valid ? 'fa-circle-check' : 'fa-circle-xmark'}"></i>`;
      const reasonEl = document.getElementById('certificateRevokedReason');
      reasonEl.textContent = reason ? `Motivo: ${reason}` : '';
      reasonEl.classList.toggle('hidden', valid || !reason);
      document.getElementById('certificatePrintBtn').classList.toggle('hidden', !valid);
      document.getElementById('certificateDownloadBtn')?.classList.toggle('hidden', !valid);

      const qr = document.getElementById('certificateQr');
      qr.innerHTML = '';
      if(code) await ensureQrCodeV23();
    ...
    async function verifyPublicCertificate(code, { updateField = true } = {}) {
      ...
      if (status) status.textContent = cert.is_valid ? 'Certificado válido.' : 'El certificado existe, pero está revocado.';
      showCertificate(cert, { public: true });
      return cert;
    }
    ...
    async function downloadCurrentCertificatePdf() {
      ...
      if (status !== 'valid') { showToast('Un certificado revocado no se puede descargar como válido.'); return; }
      await ensureJsPdfV23(); if (!window.jspdf?.jsPDF) { showToast('No se pudo cargar el generador de PDF.'); return; }
      ...
      doc.text('Lutmin certifica que', W / 2, 57, { align: 'center' });
      ...
      doc.setTextColor(71, 85, 105); doc.setFont('helvetica', 'normal'); doc.setFontSize(12);
      doc.text('ha aprobado satisfactoriamente el curso', W / 2, 97, { align: 'center' });
      ...
      doc.text(`Calificación: ${score}%`, 130, 143);
      ...
      doc.setFontSize(7); doc.text('La autenticidad de este certificado puede verificarse mediante el código o QR.', 24, 192);
    ...
    function printCurrentCertificate() {
      ...
      if (!c) return;
      const status = c.status || c.certificate_status || 'valid';
      if (status !== 'valid') { showToast('Un certificado revocado no se puede imprimir como válido.'); return; }
      const code = c.code || c.certificate_code || '';
      const fullName = c.full_name || 'Alumno';
      const course = c.course_title || 'Curso Lutmin';
      const hours = Number(c.duration_hours || 0);
      ...
      w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>Certificado ${escapeHtml(code)}</title><style>@page{size:A4 landscape;margin:12mm}body{font-family:Arial,sans-serif;margin:0;color:#000B3C}.sheet{border:4px solid #000B3C;min-height:175mm;padding:18mm;box-sizing:border-box;display:flex;flex-direction:column;justify-content:center;text-align:center}.logo{height:55px;object-fit:contain;margin:auto}.eyebrow{margin-top:24px;letter-spacing:.22em;font-weight:700;font-size:12px;color:#2F8DFD}.name{font-size:38px;font-weight:800;margin:22px 0 8px}.course{font-size:30px;font-weight:800;margin:10px}.meta{font-size:15px;color:#475569;margin-top:18px;line-height:1.8}.code{margin-top:28px;font-size:11px;color:#64748b}.verify{font-size:9px;word-break:break-all;color:#94a3b8;margin-top:6px}</style></head><body><div class="sheet"><img class="logo" src="${logo}" loading="lazy" decoding="async"><div class="eyebrow">CERTIFICADO DE FORMACIÓN</div><p>Se certifica que</p><div class="name">${escapeHtml(fullName)}</div><p>ha aprobado satisfactoriamente el curso</p><div class="course">${escapeHtml(course)}</div><div class="meta">Carga horaria: <strong>${hours} h</strong> · Calificación: <strong>${score}%</strong> · Emitido: <strong>${issued}</strong></div>${qrImg ? `<img src="${qrImg}" style="width:110px;height:110px;margin:24px auto 0" loading="lazy" decoding="async">` : ''}<div class="code">Código verificable: <strong>${escapeHtml(code)}</strong></div><div class="verify">${escapeHtml(verify)}</div></div><script>window.onload=()=>setTimeout(()=>window.print(),500)<\/script>
</body></html>`);
      w.document.close();
    }
    ...
    function goToCampusTab(tab) {
      if (tab === 'admin' && currentLutminUser?.role !== 'admin') tab = currentLutminUser?.role === 'company_admin' ? 'company' : 'dashboard';
      if (tab === 'company' && currentLutminUser?.role !== 'company_admin') tab = 'dashboard';
      if (tab === 'talent' && currentLutminUser?.role !== 'student') tab = currentLutminUser?.role === 'company_admin' ? 'company' : 'dashboard';
      if (currentLutminUser?.role === 'company_admin' && !['company','profile','notifications','support'].includes(tab)) tab = 'company';
    ...
    function renderLessonVideo(lesson) {
      ...
      const safe = safeExternalUrl(lesson.video_url);
      if (!safe) {
      ...
      const embed = getVideoEmbed(safe);
      if (embed) {
      ...
      if (/\.(mp4|webm|ogg)(\?.*)?$/i.test(safe)) {
        area.innerHTML = `<video class="w-full h-full" controls preload="metadata"><source src="${escapeHtml(safe)}">Tu navegador no puede reproducir este video.</video>`;
        return;
      }
    ...
    function isLessonUnlocked(summary, lesson) {
      if (!summary || !lesson) return false;
      if (summary.completedSet.has(lesson.id)) return true;
      return summary.nextLesson?.id === lesson.id;
    }
    ...
    async function openLessonById(lessonId) {
      ...
      materialArea.innerHTML = material
        ? `<a href="${escapeHtml(material)}" target="_blank" rel="noopener noreferrer" class="inline-flex w-full items-center justify-center gap-2 px-4 py-3 rounded-xl bg-slate-100 hover:bg-slate-200 text-lutmin-dark font-bold text-sm"><i class="fa-solid fa-file-arrow-down"></i>Abrir material</a>`
    ...
    async function downloadCompanyExecutivePdf(){
      if(!companyPortalData?.company)return showToast('No hay empresa cargada.'); await ensureJsPdfV23(); if(!window.jspdf?.jsPDF)return showToast('No pude iniciar el generador PDF.');
    ...
    async function loadAdminData() {
      ...
      const [profilesRes, coursesRes, lessonsRes, enrollmentsRes, progressRes, assessmentsRes, questionsRes, attemptsRes, certificatesRes, notesRes, offeringsRes, leadsRes, paymentsRes, accessRolesRes] = await Promise.all([
        ...
        supabaseClient.from('courses').select('id,slug,title,description,duration_hours,level,category,published,public_featured,created_at,updated_at').order('created_at', { ascending: false }),
        supabaseClient.from('lessons').select('id,course_id,title,description,content,duration_minutes,sort_order,video_url,material_url,material_path,updated_at').order('sort_order'),
        ...
        supabaseClient.from('assessments').select('id,course_id,title,description,passing_score,published,created_at').order('created_at', { ascending: false }),
        supabaseClient.from('assessment_questions').select('id,assessment_id,question_text,sort_order,points').order('sort_order'),
        ...
        supabaseClient.from('certificates').select('id,user_id,course_id,code,full_name,course_title,duration_hours,score,status,issued_at,revoked_at,revoked_reason').order('issued_at', { ascending: false }),
      ...
      if (firstError) {
        ...
        showToast('No pude cargar Administración. Verificá que hayas ejecutado los SQL hasta la Etapa 12.');
        return;
      }
      ...
      renderAdminPanel();
      await loadAdminCompaniesData();
      await loadAdminTrainingData();
      await loadAdminV1Ops();
      await loadAdminConectaData();
      await loadAdminOpsV16();
      await loadExecutiveV17();
      await loadAdminV18();
      await loadAdminWorkspacePrefsV19();
    ...
    async function loadAdminCompaniesData() {
      ...
      if (companiesRes.error || membersRes.error) {
        ...
        showToast('No pude cargar Empresas. Ejecutá el SQL de la Etapa 13.');
        return;
      }
    ...
    async function loadAdminTrainingPlans(){
      ...
      if(plansRes.error||coursesRes.error){console.error(plansRes.error||coursesRes.error);showToast('No pude cargar los planes corporativos. Ejecutá el SQL V1.4.');return;}
    ...
    async function loadSupportCenter(){
      ...
      if(tRes.error||mRes.error){console.error(tRes.error||mRes.error);return showToast('No pude cargar la Mesa de ayuda. Ejecutá el SQL V1.3.');}
    ...
    function renderAdminCourses() {
      ...
      container.innerHTML = adminCourses.map(course => {
        ...
        const lessonPreview = lessons.length
          ? lessons.slice(0, 5).map(l => `<button onclick="openAdminLessonEditor('${l.id}')" class="text-xs px-2.5 py-1 rounded-full bg-slate-100 hover:bg-blue-50 hover:text-blue-700 text-slate-600">${l.sort_order}. ${escapeHtml(l.title)}${l.material_path ? ' · 📎' : ''}</button>`).join('')
        ...
        return `<div class="p-5 sm:p-6">
          <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
            <div class="min-w-0">
              ...
              <div class="mt-3 flex flex-wrap gap-2">${lessonPreview}${lessons.length > 5 ? `<span class="text-xs text-slate-400">+${lessons.length - 5} más</span>` : ''}</div>
            </div>
    ...
    function renderAdminCertificates() {
      ...
      list.innerHTML = adminCertificates.map(cert => {
        ...
        return `<div class="p-5 sm:p-6 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div class="min-w-0"><div class="flex flex-wrap gap-2 items-center"><p class="font-extrabold text-lutmin-dark">${escapeHtml(cert.full_name)}</p><span class="text-[10px] px-2.5 py-1 rounded-full font-bold ${valid ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}">${valid ? 'Válido' : 'Revocado'}</span></div><p class="mt-1 text-sm text-slate-500">${escapeHtml(cert.course_title)} · ${Math.round(Number(cert.score))}% · ${formatCertificateDate(cert.issued_at)}</p><p class="mt-2 text-[10px] text-slate-400 break-all">${escapeHtml(cert.code)}</p>${cert.revoked_reason ? `<p class="mt-1 text-xs text-red-500">${escapeHtml(cert.revoked_reason)}</p>` : ''}</div>
          <div class="flex flex-wrap gap-2 shrink-0"><button onclick="showAdminCertificate('${cert.id}')" class="px-3 py-2 rounded-xl bg-slate-100 text-xs font-bold">Ver</button>${valid ? `<button onclick="revokeAdminCertificate('${cert.id}')" class="px-3 py-2 rounded-xl bg-red-50 text-red-600 text-xs font-bold">Revocar</button>` : `<button onclick="restoreAdminCertificate('${cert.id}')" class="px-3 py-2 rounded-xl bg-green-50 text-green-700 text-xs font-bold">Restaurar</button>`}</div>
        </div>`;
      }).join('');
    }
    ...
    async function loadAdminV1Ops() {
      ...
      if (controlRes.error || auditRes.error || announcementRes.error) {
        ...
        showToast('Falta instalar los componentes de LUTMIN V1.0.');
        return;
      }
    ...
    function jobStatusLabel(value) {
      return ({draft:'Borrador',published:'Publicada',closed:'Cerrada',new:'Nueva',reviewing:'En revisión',interview:'Entrevista',rejected:'Finalizada',hired:'Seleccionado/a',withdrawn:'Retirada'})[value] || value || '';
    }
    ...
    async function openConectaCampus(){
      if(!supabaseClient)return; const {data:{session}}=await supabaseClient.auth.getSession();
      if(!session){openAuthModal('campus');document.getElementById('authStatus').textContent='Ingresá a tu Campus para crear tu perfil profesional.';return;}
    ...
    async function loadTalentCenter(){
      ...
      const [profileRes,skillsRes,expRes,appsRes,certRes,jobsRes]=await Promise.all([
      ...
      const err=[profileRes,skillsRes,expRes,appsRes,certRes,jobsRes].find(x=>x.error)?.error; if(err){console.error(err);return showToast('No pude cargar Lutmin Conecta. Ejecutá el SQL de V1.2.');}
      talentData={profile:profileRes.data||null,skills:skillsRes.data||[],experiences:expRes.data||[],applications:appsRes.data||[],certificates:certRes.data||[],jobs:Array.isArray(jobsRes.data)?jobsRes.data:[]};
      renderTalentCenter();
    }
    function renderTalentCenter(){
      const p=talentData.profile||{};
      const set=(id,v)=>{const el=document.getElementById(id);if(el)el.value=v??'';}; set('talentHeadline',p.headline);set('talentCity',p.city);set('talentProvince',p.province);set('talentPhone',p.phone);set('talentLinkedin',p.linkedin_url);set('talentYears',p.years_experience||0);set('talentAvailability',p.availability||'available');set('talentBio',p.bio);
      document.getElementById('talentTravel').checked=!!p.willing_travel;document.getElementById('talentVisible').checked=!!p.visible;document.getElementById('talentShareContact').checked=p.share_contact_with_companies!==false;
      const badge=document.getElementById('talentVisibilityBadge');badge.textContent=p.visible?'Visible para empresas':'Privado';badge.className=`px-3 py-1 rounded-full text-[10px] font-bold ${p.visible?'bg-green-50 text-green-700':'bg-slate-100 text-slate-600'}`;
    ...
    window.addEventListener('load', async () => {
      if (!supabaseClient) return;
      await Promise.all([loadPublicCatalog(), loadPublicSiteStatsV21()]);
    ...
    async function downloadExecutiveOpsPdfV16(){
      await ensureJsPdfV23();if(!window.jspdf?.jsPDF){showToast('No pude cargar el generador PDF.');return;}
    ...
    async function loadAdminV18(){
      ...
      if(err){console.warn('V1.8 no disponible:',err.message);const root=document.getElementById('healthFindingsV18');if(root)root.innerHTML='<div class="p-4 rounded-2xl bg-amber-50 text-amber-800 text-sm">Ejecutá el SQL MASTER de V1.8. Ese mismo archivo repara el error de V1.6.</div>';return;}
    ...
    const ADMIN_MODULES_V19 = {
      ...
      website:{title:'Sitio web',description:'Contenido público, identidad visual y configuración del Inicio.'},
    ...
    function tagAdminBlocksV19(){
      ...
      [...admin.children].forEach(el=>{
        ...
        if(el.dataset.adminModuleV19) return;
    ...
    async function runAdminDiagnosticsV19(){
      ...
      if(error){root.innerHTML=`<div class="rounded-xl bg-red-50 text-red-700 p-3 text-xs"><strong>No pude ejecutar el diagnóstico.</strong><p class="mt-1">${escapeHtml(error.message)}</p><p class="mt-2">Ejecutá el SQL MASTER V1.9 completo.</p></div>`;return;}
...
// ===== V2.2 REUBICADO CORRECTAMENTE POR V2.3 =====
// =========================================================
// LUTMIN V2.2 · CMS + RUTAS + ENCUESTAS + LISTA DE ESPERA
// =========================================================
let siteConfigV22=null, adminPathsV22=[], adminPathCoursesV22=[], adminPathAssignmentsV22=[];

function slugV22(v){return String(v||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'');}
function publicSectionV22(id,show){const el=document.getElementById(id);if(el)el.classList.toggle('hidden',!show);}

async function loadPublicSiteConfigV22(){
  if(!supabaseClient)return;
  const cached=lutminCacheGet('public-site-config-v23',300000);
  const result=cached?{data:cached,error:null}:await supabaseClient.from('public_site_config').select('*').eq('id',1).maybeSingle();
  const {data,error}=result;
  if(error||!data)return;
  if(!cached)lutminCacheSet('public-site-config-v23',data);
  siteConfigV22=data; applyPublicSiteConfigV22(data);
}
function applyPublicSiteConfigV22(c){
  document.body.classList.remove('site-preset-minimal','site-preset-editorial','site-preset-institucional');
  document.body.classList.add(`site-preset-${c.preset||'institucional'}`);
  const logo=document.getElementById('publicHeaderLogoV22');if(logo){logo.style.height=`${Number(c.logo_height||72)}px`;logo.style.transform='none';}
  const kicker=document.getElementById('publicHeroKickerV22');if(kicker)kicker.innerHTML=`<span class="w-2 h-2 rounded-full bg-lutmin-light shadow-[0_0_0_5px_rgba(47,141,253,.10)]"></span>${escapeHtml(c.hero_kicker||'')}`;
  const title=document.getElementById('publicHeroTitleV22');if(title)title.textContent=c.hero_title||'';
  const hi=document.getElementById('publicHeroHighlightV22');if(hi)hi.textContent=c.hero_highlight||'';
  const sub=document.getElementById('publicHeroSubtitleV22');if(sub)sub.textContent=c.hero_subtitle||'';
  const p=document.getElementById('publicPrimaryCtaV22');if(p)p.childNodes[0].textContent=(c.primary_cta_label||'Ver capacitaciones')+' ';
  const q=document.getElementById('publicSecondaryCtaV22');if(q)q.textContent=c.secondary_cta_label||'Capacitar una empresa';
  const metric=document.getElementById('publicStatsV21');if(metric)metric.classList.toggle('hidden',c.show_public_metrics===false);
  publicSectionV22('faq',c.show_faq!==false); publicSectionV22('certificados-publicos',c.show_certificates!==false);
  const eco=document.querySelector('[data-public-ecosystem-v21]');if(eco)eco.classList.toggle('hidden',c.show_ecosystem===false);
}

async function loadSiteEditorV22(){
  if(!supabaseClient||currentLutminUser?.role!=='admin')return;
  let c=siteConfigV22; if(!c){const r=await supabaseClient.from('public_site_config').select('*').eq('id',1).maybeSingle();if(r.error)return showToast('No pude cargar la configuración web.');c=r.data;siteConfigV22=c;}
  const set=(id,v)=>{const e=document.getElementById(id);if(e)e.value=v??''};
  set('sitePresetV22',c.preset);set('siteLogoHeightV22',c.logo_height);set('siteKickerV22',c.hero_kicker);set('siteTitleV22',c.hero_title);set('siteHighlightV22',c.hero_highlight);set('siteSubtitleV22',c.hero_subtitle);set('sitePrimaryV22',c.primary_cta_label);set('siteSecondaryV22',c.secondary_cta_label);
  [['siteShowMetricsV22','show_public_metrics'],['siteShowFaqV22','show_faq'],['siteShowCertsV22','show_certificates'],['siteShowEcoV22','show_ecosystem']].forEach(([id,k])=>{const e=document.getElementById(id);if(e)e.checked=c[k]!==false;});
}
document.getElementById('siteEditorFormV22')?.addEventListener('submit',async e=>{e.preventDefault();if(currentLutminUser?.role!=='admin')return;const payload={id:1,preset:document.getElementById('sitePresetV22').value,logo_height:Number(document.getElementById('siteLogoHeightV22').value||72),hero_kicker:document.getElementById('siteKickerV22').value.trim(),hero_title:document.getElementById('siteTitleV22').value.trim(),hero_highlight:document.getElementById('siteHighlightV22').value.trim(),hero_subtitle:document.getElementById('siteSubtitleV22').value.trim(),primary_cta_label:document.getElementById('sitePrimaryV22').value.trim(),secondary_cta_label:document.getElementById('siteSecondaryV22').value.trim(),show_public_metrics:document.getElementById('siteShowMetricsV22').checked,show_faq:document.getElementById('siteShowFaqV22').checked,show_certificates:document.getElementById('siteShowCertsV22').checked,show_ecosystem:document.getElementById('siteShowEcoV22').checked,updated_by:currentLutminUser.id,updated_at:new Date().toISOString()};const {data,error}=await supabaseClient.from('public_site_config').upsert(payload).select().single();if(error)return showToast('No pude guardar el sitio: '+error.message);siteConfigV22=data;lutminCacheSet('public-site-config-v23',data);applyPublicSiteConfigV22(data);showToast('Cambios publicados en el Inicio.');});

// -------------------------- RUTAS --------------------------
async function loadAdminPathsV22(){
  if(!supabaseClient||currentLutminUser?.role!=='admin')return;
  const [p,c,a]=await Promise.all([supabaseClient.from('learning_paths').select('*').order('created_at',{ascending:false}),supabaseClient.from('learning_path_courses').select('*,course:courses(id,title,duration_hours)').order('position'),supabaseClient.from('learning_path_assignments').select('*,profile:profiles(id,full_name,email)').order('assigned_at',{ascending:false})]);
  const err=[p,c,a].find(x=>x.error)?.error;if(err){console.error(err);return;}
  adminPathsV22=p.data||[];adminPathCoursesV22=c.data||[];adminPathAssignmentsV22=a.data||[];renderAdminPathsV22();
}
function renderAdminPathsV22(){
  const root=document.getElementById('adminPathsListV22');if(!root)return;
  if(!adminPathsV22.length){root.innerHTML='<div class="p-5 rounded-2xl bg-slate-50 text-sm text-slate-500">Todavía no creaste rutas.</div>';return;}
  const courseOptions=(adminCourses||[]).filter(c=>c.published!==false).map(c=>`<option value="${c.id}">${escapeHtml(c.title)}</option>`).join('');
  const studentOptions=(adminProfiles||[]).filter(p=>typeof adminIsStudent==='function'?adminIsStudent(p):p.role==='student').map(p=>`<option value="${p.id}">${escapeHtml(p.full_name||p.email)}</option>`).join('');
  root.innerHTML=adminPathsV22.map(path=>{const pcs=adminPathCoursesV22.filter(x=>x.path_id===path.id).sort((a,b)=>a.position-b.position);const ass=adminPathAssignmentsV22.filter(x=>x.path_id===path.id);return `<div class="rounded-2xl border border-slate-200 p-4"><div class="flex items-start justify-between gap-3"><div><h4 class="font-extrabold text-lutmin-dark">${escapeHtml(path.title)}</h4><p class="mt-1 text-xs text-slate-500">${escapeHtml(path.description||'')} · ${pcs.length} cursos · ${ass.length} asignaciones</p></div><button onclick="deletePathV22('${path.id}')" class="text-xs text-red-500 font-bold">Eliminar</button></div><div class="mt-3 flex flex-wrap gap-2">${pcs.map(pc=>`<span class="px-3 py-1.5 rounded-full bg-violet-50 text-violet-700 text-[10px] font-bold">${pc.position}. ${escapeHtml(pc.course?.title||'Curso')} <button onclick="removeCourseFromPathV22('${pc.id}')" class="ml-1">×</button></span>`).join('')||'<span class="text-xs text-slate-400">Sin cursos todavía</span>'}</div><div class="mt-4 grid sm:grid-cols-[1fr_auto] gap-2"><select id="pathCourse-${path.id}" class="px-3 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-xs"><option value="">Agregar curso...</option>${courseOptions}</select><button onclick="addCourseToPathV22('${path.id}')" class="px-4 py-2.5 rounded-xl bg-violet-100 text-violet-700 text-xs font-extrabold">Agregar</button></div><div class="mt-2 grid sm:grid-cols-[1fr_auto] gap-2"><select id="pathStudent-${path.id}" class="px-3 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-xs"><option value="">Asignar a alumno...</option>${studentOptions}</select><button onclick="assignPathV22('${path.id}')" class="px-4 py-2.5 rounded-xl bg-lutmin-dark text-white text-xs font-extrabold">Asignar ruta</button></div></div>`}).join('');
}
document.getElementById('createPathFormV22')?.addEventListener('submit',async e=>{e.preventDefault();const title=document.getElementById('pathTitleV22').value.trim();if(!title)return;const {error}=await supabaseClient.from('learning_paths').insert({title,slug:`${slugV22(title)}-${Date.now().toString(36)}`,description:document.getElementById('pathDescriptionV22').value.trim(),created_by:currentLutminUser.id});if(error)return showToast(error.message);e.target.reset();showToast('Ruta creada.');await loadAdminPathsV22();});
async function addCourseToPathV22(pathId){const select=document.getElementById(`pathCourse-${pathId}`);const courseId=select?.value;if(!courseId)return;const position=adminPathCoursesV22.filter(x=>x.path_id===pathId).length+1;const {error}=await supabaseClient.from('learning_path_courses').upsert({path_id:pathId,course_id:courseId,position},{onConflict:'path_id,course_id'});if(error)return showToast(error.message);await loadAdminPathsV22();}
async function removeCourseFromPathV22(id){await supabaseClient.from('learning_path_courses').delete().eq('id',id);await loadAdminPathsV22();}
async function deletePathV22(id){if(!confirm('¿Eliminar esta ruta?'))return;const {error}=await supabaseClient.from('learning_paths').delete().eq('id',id);if(error)return showToast(error.message);await loadAdminPathsV22();}
async function assignPathV22(pathId){const userId=document.getElementById(`pathStudent-${pathId}`)?.value;if(!userId)return;const {error}=await supabaseClient.from('learning_path_assignments').upsert({path_id:pathId,user_id:userId,assigned_by:currentLutminUser.id},{onConflict:'path_id,user_id'});if(error)return showToast(error.message);const courseIds=adminPathCoursesV22.filter(x=>x.path_id===pathId).sort((a,b)=>a.position-b.position).map(x=>x.course_id);for(const courseId of courseIds){await supabaseClient.from('enrollments').upsert({user_id:userId,course_id:courseId,status:'active',price_amount:0,payment_currency:'ARS',payment_status:'not_required',access_granted_at:new Date().toISOString()},{onConflict:'user_id,course_id'});}showToast(`Ruta asignada. ${courseIds.length} cursos quedaron vinculados.`);await loadAdminPathsV22();}
async function loadStudentPathsV22(){if(!supabaseClient||!currentLutminUser?.id)return;const {data:as,error}=await supabaseClient.from('learning_path_assignments').select('id,path_id,path:learning_paths(id,title,description)').eq('user_id',currentLutminUser.id);const root=document.getElementById('studentPathsV22');if(error){root.innerHTML='<div class="p-5 rounded-2xl bg-amber-50 text-amber-700 text-sm">Rutas todavía no disponibles.</div>';return;}const pathIds=(as||[]).map(x=>x.path_id);let pcs=[];if(pathIds.length){const r=await supabaseClient.from('learning_path_courses').select('*,course:courses(id,title)').in('path_id',pathIds).order('position');pcs=r.data||[];}if(!(as||[]).length){root.innerHTML='<div class="lg:col-span-2 p-7 rounded-3xl bg-white border border-slate-100"><h3 class="font-extrabold text-lutmin-dark">Todavía no tenés rutas asignadas</h3><p class="mt-2 text-sm text-slate-500">Tus cursos individuales siguen disponibles normalmente.</p></div>';return;}root.innerHTML=as.map(a=>{const courses=pcs.filter(x=>x.path_id===a.path_id);const completed=courses.filter(pc=>{const s=campusCourseSummaries.find(x=>x.course.id===pc.course_id);return s?.passedAttempt||s?.percent===100;}).length;const pct=courses.length?Math.round(completed/courses.length*100):0;return `<div class="p-6 rounded-3xl bg-white border border-slate-100"><div class="flex items-start justify-between gap-3"><div><p class="text-[10px] uppercase tracking-widest font-bold text-violet-600">Ruta de aprendizaje</p><h3 class="mt-1 text-xl font-black text-lutmin-dark">${escapeHtml(a.path?.title||'Ruta')}</h3></div><span class="text-2xl font-black text-violet-600">${pct}%</span></div><p class="mt-2 text-sm text-slate-500">${escapeHtml(a.path?.description||'')}</p><div class="mt-4 h-2 rounded-full bg-slate-100"><div class="h-full rounded-full bg-violet-500" style="width:${pct}%"></div></div><div class="mt-4 space-y-2">${courses.map((pc,i)=>{const sm=campusCourseSummaries.find(x=>x.course.id===pc.course_id);const done=!!(sm?.passedAttempt||sm?.percent===100);return `<div class="flex items-center justify-between p-3 rounded-xl ${done?'bg-green-50':'bg-slate-50'} text-xs"><span><strong>${i+1}.</strong> ${escapeHtml(pc.course?.title||'Curso')}</span><span class="font-bold ${done?'text-green-600':'text-slate-400'}">${done?'Completado':`${sm?.percent||0}%`}</span></div>`}).join('')}</div></div>`}).join('');}

// -------------------------- ENCUESTAS --------------------------
async function loadAdminSurveysV22(){if(!supabaseClient||currentLutminUser?.role!=='admin')return;const select=document.getElementById('surveyCourseV22');if(select)select.innerHTML='<option value="">Elegir curso...</option>'+((adminCourses||[]).map(c=>`<option value="${c.id}">${escapeHtml(c.title)}</option>`).join(''));const [s,r]=await Promise.all([supabaseClient.from('course_surveys').select('*,course:courses(id,title)').order('created_at',{ascending:false}),supabaseClient.from('course_survey_responses').select('*').order('submitted_at',{ascending:false})]);const root=document.getElementById('adminSurveyResultsV22');if(s.error||r.error){root.innerHTML='<p class="text-sm text-amber-600">Encuestas todavía no disponibles.</p>';return;}const surveys=s.data||[],responses=r.data||[];root.innerHTML=surveys.length?surveys.map(sv=>{const rr=responses.filter(x=>x.survey_id===sv.id);const avg=rr.filter(x=>x.rating).length?(rr.reduce((a,x)=>a+Number(x.rating||0),0)/rr.filter(x=>x.rating).length).toFixed(1):'-';const nps=rr.filter(x=>x.nps!==null).length?(rr.reduce((a,x)=>a+Number(x.nps||0),0)/rr.filter(x=>x.nps!==null).length).toFixed(1):'-';return `<div class="p-4 rounded-2xl border border-slate-200 mb-3"><div class="flex justify-between"><div><h4 class="font-extrabold text-lutmin-dark">${escapeHtml(sv.course?.title||'Curso')}</h4><p class="text-xs text-slate-500">${rr.length} respuestas · Satisfacción ${avg}/5 · NPS prom. ${nps}/10</p></div><span class="text-[10px] font-bold ${sv.active?'text-green-600':'text-slate-400'}">${sv.active?'ACTIVA':'PAUSADA'}</span></div>${rr.filter(x=>x.comment).slice(0,3).map(x=>`<p class="mt-2 p-2 rounded-lg bg-slate-50 text-[11px] text-slate-600">“${escapeHtml(x.comment)}”</p>`).join('')}</div>`}).join(''):'<p class="text-sm text-slate-500">Todavía no configuraste encuestas.</p>';}
document.getElementById('surveyConfigFormV22')?.addEventListener('submit',async e=>{e.preventDefault();const courseId=document.getElementById('surveyCourseV22').value;if(!courseId)return;const payload={course_id:courseId,title:document.getElementById('surveyTitleV22').value.trim()||'Encuesta de satisfacción',active:document.getElementById('surveyActiveV22').checked,created_by:currentLutminUser.id,updated_at:new Date().toISOString()};const {error}=await supabaseClient.from('course_surveys').upsert(payload,{onConflict:'course_id'});if(error)return showToast(error.message);showToast('Encuesta configurada.');await loadAdminSurveysV22();});
async function loadStudentSurveysV22(){if(!supabaseClient||!currentLutminUser?.id)return;const root=document.getElementById('studentSurveysV22');const courseIds=campusCourseSummaries.map(x=>x.course.id);if(!courseIds.length){root.innerHTML='<div class="p-6 rounded-3xl bg-white border border-slate-100 text-sm text-slate-500">No tenés cursos para evaluar todavía.</div>';return;}const [s,r]=await Promise.all([supabaseClient.from('course_surveys').select('*,course:courses(id,title)').in('course_id',courseIds).eq('active',true),supabaseClient.from('course_survey_responses').select('*').eq('user_id',currentLutminUser.id)]);if(s.error||r.error){root.innerHTML='<div class="p-6 rounded-3xl bg-amber-50 text-amber-700 text-sm">Encuestas todavía no disponibles.</div>';return;}const answered=new Set((r.data||[]).map(x=>x.survey_id));const eligible=(s.data||[]).filter(sv=>campusCertificates.some(c=>c.course_id===sv.course_id&&c.status==='valid'));if(!eligible.length){root.innerHTML='<div class="p-6 rounded-3xl bg-white border border-slate-100"><h3 class="font-extrabold text-lutmin-dark">No tenés encuestas pendientes</h3><p class="mt-2 text-sm text-slate-500">Aparecen después de completar una capacitación.</p></div>';return;}root.innerHTML=eligible.map(sv=>answered.has(sv.id)?`<div class="p-5 rounded-3xl bg-green-50 border border-green-100"><p class="font-extrabold text-green-700">${escapeHtml(sv.course?.title||'Curso')}</p><p class="mt-1 text-xs text-green-600">Gracias. Ya recibimos tu devolución.</p></div>`:`<form onsubmit="submitSurveyV22(event,'${sv.id}')" class="p-6 rounded-3xl bg-white border border-slate-100"><h3 class="text-lg font-black text-lutmin-dark">${escapeHtml(sv.course?.title||'Curso')}</h3><p class="mt-1 text-sm text-slate-500">${escapeHtml(sv.intro||'Tu opinión nos ayuda a mejorar.')}</p><div class="mt-5 grid sm:grid-cols-2 gap-4"><div><label class="text-xs font-bold">Satisfacción general · 1 a 5</label><select name="rating" class="mt-2 w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200"><option value="5">5 · Excelente</option><option value="4">4 · Muy buena</option><option value="3">3 · Buena</option><option value="2">2 · Regular</option><option value="1">1 · Mala</option></select></div><div><label class="text-xs font-bold">¿Recomendarías Lutmin? · 0 a 10</label><input name="nps" type="number" min="0" max="10" value="10" class="mt-2 w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200"></div></div><textarea name="comment" rows="3" placeholder="Comentario opcional" class="mt-4 w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200"></textarea><button class="mt-4 px-5 py-3 rounded-xl bg-lutmin-light text-white font-extrabold">Enviar devolución</button></form>`).join('');}
async function submitSurveyV22(event,surveyId){event.preventDefault();const f=new FormData(event.target);const payload={survey_id:surveyId,user_id:currentLutminUser.id,rating:Number(f.get('rating')),nps:Number(f.get('nps')),comment:String(f.get('comment')||'').trim()};const {error}=await supabaseClient.from('course_survey_responses').insert(payload);if(error)return showToast(error.message);showToast('Gracias por tu devolución.');await loadStudentSurveysV22();}

// -------------------------- LISTA DE ESPERA --------------------------
function ensureWaitlistModalV22(){if(document.getElementById('waitlistModalV22'))return;document.body.insertAdjacentHTML('beforeend',`<div id="waitlistModalV22" class="hidden fixed inset-0 z-[210] bg-slate-950/80 backdrop-blur-md p-4 flex items-center justify-center"><div class="w-full max-w-lg bg-white rounded-[2rem] shadow-2xl relative p-6 sm:p-8"><button onclick="closeModal('waitlistModalV22')" class="absolute right-4 top-4 w-10 h-10 rounded-full bg-slate-100 text-slate-500"><i class="fa-solid fa-xmark"></i></button><p class="text-[10px] uppercase tracking-widest font-extrabold text-amber-600">Edición completa</p><h2 class="mt-2 text-2xl font-black text-lutmin-dark">Sumarme a la lista de espera</h2><p class="mt-2 text-sm text-slate-500">Si se libera un lugar o abrimos otra edición, el equipo de Lutmin puede contactarte.</p><form id="waitlistFormV22" class="mt-6 space-y-3"><input id="waitlistCourseV22" type="hidden"><input id="waitlistOfferingV22" type="hidden"><input id="waitlistNameV22" required placeholder="Nombre completo" class="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200"><input id="waitlistEmailV22" type="email" required placeholder="Correo" class="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200"><input id="waitlistPhoneV22" placeholder="Teléfono / WhatsApp" class="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200"><button class="w-full py-3 rounded-xl bg-amber-500 text-white font-extrabold">Quiero quedar en espera</button></form></div></div>`);document.getElementById('waitlistFormV22').addEventListener('submit',submitWaitlistV22);}
function openWaitlistV22(courseId,offeringId){ensureWaitlistModalV22();document.getElementById('waitlistCourseV22').value=courseId;document.getElementById('waitlistOfferingV22').value=offeringId||'';closeModal('courseModal');openModal('waitlistModalV22');}
async function submitWaitlistV22(e){e.preventDefault();const {data,error}=await supabaseClient.rpc('join_course_waitlist',{p_course_id:document.getElementById('waitlistCourseV22').value,p_offering_id:document.getElementById('waitlistOfferingV22').value||null,p_full_name:document.getElementById('waitlistNameV22').value.trim(),p_email:document.getElementById('waitlistEmailV22').value.trim(),p_phone:document.getElementById('waitlistPhoneV22').value.trim()});if(error||!data?.ok)return showToast(data?.error||error?.message||'No pude registrarte.');closeModal('waitlistModalV22');showToast('Listo. Quedaste en la lista de espera.');}
async function loadAdminWaitlistV22(){if(!supabaseClient||currentLutminUser?.role!=='admin')return;const {data,error}=await supabaseClient.from('course_waitlist').select('*,course:courses(id,title),offering:course_offerings(id,start_date)').order('created_at',{ascending:false});const root=document.getElementById('adminWaitlistListV22');if(error){root.innerHTML='<div class="p-6 text-sm text-amber-700">Lista de espera todavía no disponible.</div>';return;}root.innerHTML=(data||[]).length?(data||[]).map(x=>`<div class="p-5 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3"><div><p class="font-extrabold text-lutmin-dark">${escapeHtml(x.full_name)}</p><p class="mt-1 text-xs text-slate-500">${escapeHtml(x.email)} · ${escapeHtml(x.phone||'Sin teléfono')}</p><p class="mt-1 text-xs text-slate-500">${escapeHtml(x.course?.title||'Curso')}${x.offering?.start_date?' · '+formatPublicDate(x.offering.start_date):''}</p></div><div class="flex items-center gap-2"><span class="px-3 py-1.5 rounded-full bg-amber-50 text-amber-700 text-[10px] font-bold">${escapeHtml(x.status)}</span><button onclick="updateWaitlistV22('${x.id}','contacted')" class="px-3 py-2 rounded-xl bg-blue-50 text-blue-700 text-xs font-bold">Contactado</button><button onclick="updateWaitlistV22('${x.id}','converted')" class="px-3 py-2 rounded-xl bg-green-50 text-green-700 text-xs font-bold">Convertido</button></div></div>`).join(''):'<div class="p-6 text-sm text-slate-500">No hay personas en lista de espera.</div>';}
async function updateWaitlistV22(id,status){const {error}=await supabaseClient.from('course_waitlist').update({status,updated_at:new Date().toISOString()}).eq('id',id);if(error)return showToast(error.message);await loadAdminWaitlistV22();}

// Lazy loading de nuevas áreas.
document.addEventListener('click',e=>{const b=e.target.closest('[data-campus-tab]');if(!b)return;const tab=b.dataset.campusTab;if(tab==='paths')setTimeout(loadStudentPathsV22,50);if(tab==='surveys')setTimeout(loadStudentSurveysV22,50);});

// Extiende el selector modular sin romper la V2.0/V2.1.
if(typeof setAdminModuleV19==='function'){
  const originalSetAdminModuleV22=setAdminModuleV19;
  setAdminModuleV19=function(module,opts={}){const out=originalSetAdminModuleV22(module,opts);if(module==='website')setTimeout(loadSiteEditorV22,30);if(module==='academic')setTimeout(()=>{loadAdminPathsV22();loadAdminSurveysV22();},30);if(module==='commercial')setTimeout(loadAdminWaitlistV22,30);return out;};
}

window.addEventListener('load',()=>lutminIdleV23(loadPublicSiteConfigV22,1100));



// =========================================================
// V2.3 · PANEL DE RENDIMIENTO LOCAL
// =========================================================
function showRuntimeDiagnosticsV23(){
  const m=window.lutminRuntimeV23?.()||{};
  const err=window.__lutminLastError;
  const msg=`Lutmin ${m.version||''}\nSolicitudes Supabase: ${m.requests||0}\nPromedio: ${m.avgRequestMs||0} ms\nSolicitudes lentas: ${(m.slowRequests||[]).length}\n${err?'Último error: '+err.message:'Sin errores JS detectados en esta sesión.'}`;
  alert(msg);
}
...
// =========================================================
// V2.3 · SALUD DE FRONTEND / RENDIMIENTO
// =========================================================
async function loadRuntimePanelV23(){
  const root=document.getElementById('runtimePanelV23'); if(!root)return;
  const m=window.lutminRuntimeV23?.()||{};
  let errors=[];
  if(currentLutminUser?.role==='admin'&&supabaseClient){
    try{const r=await supabaseClient.from('client_error_logs').select('id,message,source,line_number,app_version,created_at').order('created_at',{ascending:false}).limit(8);if(!r.error)errors=r.data||[];}catch(_){ }
  }
  root.innerHTML=`<div class="grid grid-cols-2 xl:grid-cols-4 gap-3">
    <div class="p-4 rounded-2xl bg-slate-50"><p class="text-[10px] font-bold text-slate-400">VERSIÓN</p><p class="mt-1 text-xl font-black text-lutmin-dark">${escapeHtml(m.version||'2.3')}</p></div>
    <div class="p-4 rounded-2xl bg-slate-50"><p class="text-[10px] font-bold text-slate-400">CONSULTAS</p><p class="mt-1 text-xl font-black text-lutmin-dark">${Number(m.requests||0)}</p></div>
    <div class="p-4 rounded-2xl bg-slate-50"><p class="text-[10px] font-bold text-slate-400">PROMEDIO API</p><p class="mt-1 text-xl font-black text-lutmin-dark">${Number(m.avgRequestMs||0)} ms</p></div>
    <div class="p-4 rounded-2xl ${Number((m.slowRequests||[]).length)?'bg-amber-50':'bg-green-50'}"><p class="text-[10px] font-bold text-slate-400">LENTAS &gt;1,2s</p><p class="mt-1 text-xl font-black ${Number((m.slowRequests||[]).length)?'text-amber-700':'text-green-700'}">${(m.slowRequests||[]).length}</p></div>
  </div>
  <div class="mt-4 flex flex-wrap gap-2"><button onclick="loadRuntimePanelV23()" class="px-4 py-2 rounded-xl bg-slate-100 text-xs font-bold">Actualizar</button><button onclick="saveRuntimeSnapshotV23()" class="px-4 py-2 rounded-xl bg-lutmin-dark text-white text-xs font-bold">Guardar medición</button></div>
  <div class="mt-5"><p class="text-xs font-extrabold text-lutmin-dark">Errores recientes detectados</p>${errors.length?`<div class="mt-2 space-y-2">${errors.map(e=>`<div class="p-3 rounded-xl bg-red-50/70 border border-red-100 text-xs"><strong class="text-red-700">${escapeHtml(e.message)}</strong><p class="mt-1 text-[10px] text-slate-500">${escapeHtml(e.source||'frontend')}${e.line_number?' · línea '+e.line_number:''} · ${new Date(e.created_at).toLocaleString('es-AR')}</p></div>`).join('')}</div>`:'<div class="mt-2 p-3 rounded-xl bg-green-50 text-xs text-green-700">No hay errores registrados para usuarios autenticados.</div>'}</div>`;
}
async function saveRuntimeSnapshotV23(){
  if(!supabaseClient||!currentLutminUser?.id)return;
  const metrics=window.lutminRuntimeV23?.()||{};
  const {error}=await supabaseClient.from('performance_snapshots').insert({user_id:currentLutminUser.id,app_version:LUTMIN_V23_VERSION,metrics});
  showToast(error?'No pude guardar la medición: '+error.message:'Medición de rendimiento guardada.');
}
function ensureRuntimePanelV23(){
  if(document.getElementById('runtimePanelCardV23'))return;
  const diagnostics=document.getElementById('adminDiagnosticsV19');
  const host=diagnostics?.parentElement;
  if(!host)return;
  const card=document.createElement('div'); card.id='runtimePanelCardV23'; card.className='mt-5 rounded-3xl border border-slate-200 p-5 sm:p-6 bg-white';
  card.innerHTML='<div class="flex items-start justify-between gap-3"><div><p class="text-[10px] uppercase tracking-widest font-extrabold text-lutmin-light">V2.3 · rendimiento</p><h4 class="mt-1 text-lg font-black text-lutmin-dark">Salud del navegador</h4><p class="mt-1 text-xs text-slate-500">Consultas a Supabase, solicitudes lentas y errores JavaScript de esta sesión.</p></div><button onclick="loadRuntimePanelV23()" class="px-3 py-2 rounded-xl bg-slate-100 text-xs font-bold">Medir</button></div><div id="runtimePanelV23" class="mt-4"><p class="text-xs text-slate-500">Tocá Medir para ver el estado.</p></div>';
  host.appendChild(card);
}

document.addEventListener('DOMContentLoaded',()=>{
  setTimeout(ensureRuntimePanelV23,600);
  document.querySelectorAll('img:not([loading])').forEach(img=>{if(!img.closest('header')&&!img.src.includes('LOGO.png')){img.loading='lazy';img.decoding='async';}});
  // Debounce de búsqueda global admin sin cambiar su API pública
  const q=document.getElementById('adminGlobalSearchV19');
  if(q){q.removeAttribute('oninput');q.addEventListener('input',debounceV23(()=>adminGlobalSearchV19(),180));}
});
